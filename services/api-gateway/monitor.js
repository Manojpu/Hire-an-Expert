...
function displayStatus(results) {
  ...
  results.forEach((result) => {
    ...
    console.log(
      `${statusIcon} ${result.name.padEnd(15)} :${result.port} ${responseTime}`[
        statusColor
      ]
    );
  ...
  console.log("=".repeat(50));
  console.log(
    `System Health: ${onlineCount}/${totalCount} services online (${healthPercentage}%)`[
      summaryColor
    ]
  );
  ...
  if (healthPercentage < 100) {
    ...
    results.forEach((result) => {
      if (result.status === "offline") {
        console.log(
          `   • ${result.name}: Check if service is running on port ${result.port}`
            .yellow
        );
        ...
        switch (result.name) {
          case "API Gateway":
            console.log(
              "     Command: cd services/api-gateway && npm run dev".gray
            );
            break;
          case "Message Service":
            console.log(
              "     Command: cd services/msg-service && npm run dev".gray
            );
            break;
          case "Frontend":
          ...
          case "Auth Service":
            console.log(
              "     Command: cd services/auth-service && pipenv run python main.py"
                .gray
            );
            break;
          case "Gig Service":
            console.log(
              "     Command: cd services/gig-service && python main.py".gray
            );
            break;
          case "Booking Service":
            console.log(
              "     Command: cd services/booking-service && python main.py".gray
            );
            break;
          case "Payment Service":
            console.log(
              "     Command: cd services/payment-service && python -m app.main"
                .gray
            );
            break;
        }
