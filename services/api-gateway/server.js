...
app.use('/api/gigs', authenticateJWT, createProxyMiddleware({
  target: services.gig,
  changeOrigin: true,
  pathRewrite: {
  ...
  onError: (err, req, res) => {
    ...
    res.status(503).json({ 
      ...
      message: 'The gig service is currently unavailable. Please try again later.',
      timestamp: new Date().toISOString()
    });
  },
  onProxyReq: (proxyReq, req, res) => {
...
app.use('/api/bookings', authenticateJWT, createProxyMiddleware({
  ...
  onError: (err, req, res) => {
    ...
    res.status(503).json({ 
      ...
      message: 'The booking service is currently unavailable. Please try again later.',
      timestamp: new Date().toISOString()
    });
  },
  onProxyReq: (proxyReq, req, res) => {
...
app.use('/api/message', authenticateJWT, createProxyMiddleware({
  ...
  onError: (err, req, res) => {
    ...
    res.status(503).json({ 
      ...
      message: 'The messaging service is currently unavailable. Please try again later.',
      timestamp: new Date().toISOString()
    });
  },
  onProxyReq: (proxyReq, req, res) => {
...
app.use('/api/conversations', authenticateJWT, createProxyMiddleware({
  target: services.message,
  changeOrigin: true,
  pathRewrite: {
  ...
  onError: (err, req, res) => {
    ...
    res.status(503).json({ 
      ...
      message: 'The conversation service is currently unavailable. Please try again later.',
      timestamp: new Date().toISOString()
    });
  },
  onProxyReq: (proxyReq, req, res) => {
    console.log(`Proxying to Message Service (Conversations): ${req.method} ${req.url}`);
  }
}));
...
function connectToMessageService() {
  try {
    ...
    messageServiceSocket.on('connect_error', (error) => {
      console.error('❌ Message Service Socket.IO Connection Error:', error.message);
    });
    ...
    console.error('❌ Failed to connect to Message Service Socket.IO:', error.message);
