...
import { format, addMinutes, startOfDay, addDays, isBefore, isEqual, parseISO, isAfter } from "date-fns";
import { MOCK_EXPERTS } from "@/data/mockExperts";
import { isSlotBooked, addBooking, Booking, confirmBookingWithPayment } from "@/lib/bookings";
...
const Book = () => {
  ...
  const daySlots = useMemo(() => {
    if (!selectedDate) return [] as { dt: Date; iso: string; past: boolean; booked: boolean }[];
  ...
  if (!expert) return <div className="min-h-[60vh] flex items-center justify-center">Expert not found</div>;

  const handleSelectSlot = (iso: string, slotObj: { dt: Date; booked: boolean; past: boolean }) => {
  ...
  const handleBook = () => {
    if (!selectedSlotIso) {
      toast({ title: 'Select a time slot', description: 'Please choose an available time slot before requesting.' });
      return;
    }
    if (!description.trim()) {
    ...
    toast({ title: 'Request sent', description: 'Your booking request was sent to the expert. You can pay once the expert approves.' });
    navigate('/my-bookings');
  };

  const saveForLater = () => {
    if (!selectedSlotIso) {
      toast({ title: 'Select a time slot', description: 'Please choose a time slot before saving.' });
      return;
    }
    ...
    try {
      ...
      list.push({ expertId: expert.id, date: selectedDate.toISOString(), slot: selectedSlotIso });
      localStorage.setItem(key, JSON.stringify(list));
      toast({ title: 'Saved', description: 'This booking was saved for later.' });
    } catch (error) {
      console.error('Error saving to localStorage:', error);
      toast({ title: 'Error', description: 'Unable to save for later. Please try again.' });
    }
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 container w-11/12 py-8">
          ...
          <div className="p-6 bg-gradient-to-r from-background/80 to-background/40">
            <div className="flex gap-6 items-center">
              ...
              <div className="flex-1">
                ...
                <div className="text-sm text-muted-foreground/90">{expert.title}</div>
                <div className="mt-3 text-sm font-medium text-primary/90">Select a date and available time slot below</div>
              </div>
              <div className="w-48">
                <div className="text-sm font-medium text-foreground/90 mb-1.5">Duration</div>
                <select 
                ...
                <div className="text-sm font-medium text-foreground/90 mb-1.5 mt-4">Language</div>
                <select 
                  className="w-full px-4 py-2.5 rounded-xl bg-background/80 backdrop-blur-sm border border-border/50 focus:border-primary/50 focus:ring-1 focus:ring-primary/50 transition-all" 
                  value={language || ''} 
            ...
            <div className="mt-8">
              <div className="flex gap-3 pb-4 overflow-auto scrollbar-thin scrollbar-thumb-primary/10 scrollbar-track-transparent">
                {days.map((d) => (
                  ...
                  >
                    <div className="text-sm font-semibold">{format(d, 'EEE')}</div>
                    <div className="text-xs opacity-80 mt-0.5">{format(d, 'dd MMM')}</div>
                  </button>
                ))}
              </div>
              ...
              <div className="mt-6 grid grid-cols-4 gap-3">
                {daySlots.map((s) => {
                  ...
                  return (
                    <button key={s.iso} disabled={disabled} onClick={() => handleSelectSlot(s.iso, s)} className={className}>
                      {format(s.dt, 'hh:mm a')}
                    </button>
                  );
                })}
              ...
              <div className="mt-8">
                <label className="text-sm font-medium text-foreground/90">Meeting Type</label>
              ...
              <div className="mt-8">
                <label className="text-sm font-medium text-foreground/90">Consultation Description</label>
                <textarea 
                  value={description} 
      ...
      <aside className="space-y-6">
          ...
          <div className="p-6 bg-gradient-to-b from-background/80 to-background/40">
            ...
            <div className="mt-6 space-y-4">
              <div className="p-4 rounded-xl bg-background/80 backdrop-blur-sm border border-border/30">
                <div className="text-sm font-medium text-foreground/90">Base rate</div>
                <div className="text-xl font-bold text-primary mt-1">{expert.pricing.currency} {baseRate}</div>
              </div>

              <div className="p-4 rounded-xl bg-background/80 backdrop-blur-sm border border-border/30">
                <div className="text-sm font-medium text-foreground/90">Duration</div>
              ...
              <div className="p-4 rounded-xl bg-background/80 backdrop-blur-sm border border-border/30">
                <div className="space-y-3">
                  <div>
                    <div className="text-sm font-medium text-foreground/90">Subtotal</div>
                    <div className="font-medium mt-1">Rs. {subtotal}</div>
                  </div>
                  <div>
                    <div className="text-sm text-muted-foreground">Platform fee (5%)</div>
              ...
              <div className="p-4 rounded-xl bg-gradient-to-r from-primary/10 to-background border border-primary/20">
                <div className="text-sm font-medium text-foreground/90">Total Amount</div>
                <div className="text-3xl font-bold text-primary mt-1">Rs. {total}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-background/60 backdrop-blur-sm rounded-2xl border border-border/50 shadow-lg overflow-hidden">
          <div className="p-6">
            <div className="font-medium text-foreground/90">Cancellation Policy</div>
            <div className="text-sm text-muted-foreground/90 mt-2 leading-relaxed">
              Free cancellation up to 24 hours before the session. A full refund will be provided for cancellations made within this period.
            </div>
          </div>
        </div>
      </aside>
      ...
      <Link to={`/chat/${expert.userId}`} className="fixed right-8 bottom-8 z-50">
