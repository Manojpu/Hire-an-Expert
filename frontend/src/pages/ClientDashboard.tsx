  ...
  useEffect(() => {
    try {
      ...
      const mapped = list.reduce<FavoriteType[]>((acc, i) => {
        const ex = MOCK_EXPERTS.find(
          (e) => e.id === i.expertId || e.slug === i.expertId
        );
        if (!ex) return acc;
        const title = (ex as unknown as { title?: string }).title;
        const profileImage = (ex as unknown as { profileImage?: string })
          .profileImage;
                        ...
                        >
                          <div>
                            ...
                            <div className="text-sm text-muted-foreground">
                              {format(new Date(b.dateTime), "PPP p")}
                            </div>
                            <div className="text-sm text-muted-foreground">
                              Status: {b.status}
                            </div>
          ...
          <aside className="space-y-6">
            <Card>
              ...
              <CardContent>
                {favorites.length === 0 ? (
                  <div className="text-muted-foreground">
                    You have no favorite experts yet.
                  </div>
                ) : (
                  <div className="space-y-3">
                    {favorites.map((e) => (
