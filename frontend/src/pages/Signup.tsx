...
const Signup: React.FC = () => {
    ...
    try {
      ...
      try {
        const response = await fetch(
          `${import.meta.env.VITE_USER_SERVICE_URL}/internal/users/provision`,
          {
            method: "POST",
            headers: {
        ...
        if (response.ok) {
          ...
          console.warn(
            "User profile creation failed, but Firebase signup successful"
          );
        }
      } catch (backendError) {
        console.warn(
          "User service error (Firebase signup still successful):",
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.warn(
            "User profile creation failed, but Google signup successful"
          );
        }
      } catch (backendError) {
        console.warn(
          "User service error (Google signup still successful):",
          (backendError as Error).message
        );
      }
    } catch (error) {
      ...
      setErrorMessage(
        (error as Error).message || "An error occurred during Google signup"
      );
    } finally {
      setIsSigningUp(false);
    }
  ...
  const getPasswordStrength = (password: string) => {
    ...
    if (score < 2)
      return { strength: 1, text: "Weak", color: "text-destructive" };
    if (score < 4)
      return { strength: 2, text: "Medium", color: "text-warning" };
                ...
                {formData.password && (
                  <div className="space-y-1">
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <span
                        className={cn("font-medium", passwordStrength.color)}
                      >
