...
import { ArrowLeft, Home, Calendar, BarChart3, User } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-background">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground capitalize">
                  {gig.category.replace('-', ' ')} • {gig.status}
                </p>
              </div>
            </div>
          </div>
          ...
          <nav className="flex space-x-6">
            {navigationItems.map((item) => {
              ...
              const isActive = location.pathname === item.href || 
                             (location.pathname.endsWith('/gig/' + gigId) && item.href.includes('overview'));
      ...
      <div className="container mx-auto px-4 py-6">
        <Routes>
          <Route path="overview" element={<GigOverview gig={gig} onUpdate={handleGigUpdate} />} />
          <Route path="bookings" element={<GigBookings gig={gig} />} />
          <Route path="analytics" element={<GigAnalytics gig={gig} />} />
          <Route path="profile" element={<GigProfile gig={gig} onUpdate={handleGigUpdate} />} />
